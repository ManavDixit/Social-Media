...
export const sendMessage = async ({ message, from, to, dispatch }) => {
  ...
  if (!token) {
    ...
    try {
      ...
      dispatch(setAlert({ message: `Unable to send message`, type: "error" }));
      console.log(error);
    }
  }
};
...
export const getMessages = createAsyncThunk(
  ...
  async ({ dispatch, signal, to, lastMessageCreatedAt }) => {
    ...
    if (!token) {
      ...
      try {
        let query = `${url}/message/getMessages?to=${to}&limit=5`;

        if (lastMessageCreatedAt != null) {
          query += `&cursor=${lastMessageCreatedAt}`;
        }
        ...
        if (data.success) {
          ...
          dispatch(
            setAlert({
              message: `Unable to fetch messages error ${data.error}`,
              type: "error",
            })
          );
